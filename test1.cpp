...
bool test1() {
  ...
  T1.insert(10,19);
  T1.insert(1000,12);
  cout<<T1.getValue(1000)<<" get value z t1"<<endl;
  if (T1.getValue(100) != 17 || T1.getValue(1000)!=12 || T1.getValue(10) != 19) {
    cout << "Values are not stored correctly.1" << endl;
    return false;
  }
cout<<"pierwsza czesc smiga "<<endl;

  T2.insert(2000000,39);
  T2.insert(20000,42);
  T2.insert(2,76);
  if (T2.getValue(2000000) != 39 || T2.getValue(2)!=76 || T2.getValue(20000) != 42) {
    cout << "Values are not stored correctly.2" << endl;
    return false;
  }
